...
/*******************24赛季************************************/
kb_adrust_t kb_adrust[6];
kb_adrust_t kb_slide_clamp_del;
void kb_kb_adrust_angle_change(kb_adrust_t *kb_adrust,int8_t flag,uint16_t delay_times)
{
...
void kb_adrust_angle_ctrl(void)//机械臂角度控制
{
	...
	chassis_mode == CHASSIS_EXCHANGE_MODE))
	{
		...
		if(KB_PITCH_UP)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_PITCH],-1,1);
		}
		if(KB_PITCH_DN)
		{
			kb_kb_adrust_angle_change(&kb_adrust[ADRUST_PITCH],+1,1);
