...
void Motor_pid_init(INIT_STATUS init_status) 
{
    ...
    PID_Motor_parameter_Struct.speed.p = 30;
    PID_Motor_parameter_Struct.speed.i = 0.01;
...
void Motor_angle_init()
{
	// normal_angle
	{
		...
		Motor[SLIDE_L].Angle.normal_angle = -80;//-100MAX:-850
		Motor[SLIDE_CLAMP].Angle.smallisland_angle = 0;
		Motor[EXCHANGE_YAW].Angle.normal_angle = 0;
		Motor[EXCHANGE_PITCH].Angle.normal_angle = -2000;//2200水平，MAX3500
	...
	// bigisland_slanted_angle
	{
		...
		Motor[SLIDE_CLAMP].Angle.bigisland_slanted_angle = -850;
	...
	// exchange_angle
	{
		...
		Motor[UPRISE_L].Angle.exchange_angle = 4000;
	...
	// store_angle
	{
		...
		Motor[UPRISE_L].Angle.store_angle = 1600;
		...
		Motor[EXCHANGE_PITCH].Angle.store_angle = -150;
	...
	// exchange_pick_angle
	{
		...
		Motor[CLAMP_PITCH].Angle.exchange_pick_angle = 40;
		Motor[CLAMP_ROLL].Angle.exchange_pick_angle = 0;
		Motor[SLIDE_L].Angle.exchange_pick_angle = 90;
		Motor[SLIDE_CLAMP].Angle.exchange_pick_angle = -200;
		Motor[EXCHANGE_YAW].Angle.exchange_pick_angle = 0;
		Motor[EXCHANGE_PITCH].Angle.exchange_pick_angle = -2400;
	...
	}
	{
		...
		Motor[UPRISE_L].Angle.store_smallisland_angle = 1600;
		Motor[CLAMP_YAW].Angle.store_smallisland_angle = 0;
		Motor[CLAMP_PITCH].Angle.store_smallisland_angle = -100;
		Motor[CLAMP_ROLL].Angle.store_smallisland_angle = 0;
		Motor[SLIDE_L].Angle.store_smallisland_angle = -100;
	...
	while(1)
	{
		...
		if(STAUS ==pdTRUE)
		{
			...
			if((Signal & INFO_GET_MOTOR_SIGNAL) && chassis_mode != CHASSIS_RELEASE)
			{
				...
				{
					clamp_angle_handle();
					for(ID=0;ID<MOTOR_NUMBER;ID++)//正式运行
					{
	...
	for(ID=0;ID<MOTOR_NUMBER;ID++)//堵转初始化
	{
		if(Motor[ID].MOTOR_NAME>CHASSIS_BR && Whether_Brushless_Motor(Motor[ID]))
		{
			if(Motor[ID].Brushless.offset_angle_init_flag==0)//底盘没改，不在这里进行计算
			{
				...
				if((fabs(Motor[ID].Brushless.spd_ref) - fabs(Motor[ID].Brushless.spd_fdb)) > 0.3*fabs(Motor[ID].Brushless.spd_ref))//判断是否堵转（占比越大条件越苛刻）
				{
